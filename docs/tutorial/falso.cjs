...
const ajustes = { navegador: true, theme: TEMA, palette: PALETA, bloqueoMinutos: 10, bloquearAlSuspender: true, bloquearAlBloquearWindows: true, portapapelesSegundos: 90, accesoRapido: true, arrancarConWindows: false, cerrarABandeja: true, ordenLista: 'titulo', ultimaCopia: hace(0) }
const ok = (v) => Promise.resolve(v)
const nada = () => () => {}

contextBridge.exposeInMainWorld('clac', {
  sesion: { estado: () => ok(SESION), crear: () => ok('C1-7KQ2MX-4TP9A-HX3VE-Z8R2K-N6YWD-QF5LBW'), desbloquear: () => ok(), bloquear: () => ok(), cambiarContrasena: () => ok(), claveSecreta: () => ok(''), kit: () => ok(null), restaurarElegir: () => ok(null), restaurar: () => ok() },
  ajustes: { leer: () => ok(ajustes), guardar: (c) => ok({ ...ajustes, ...c }) },
  bovedas: { listar: () => ok(bovedas), guardar: () => ok(bovedas[0]), eliminar: () => ok() },
  elementos: { listar: () => ok(lista), obtener: (i) => ok(elementos.find((e) => e.id === i)), guardar: (e) => ok(e), estado: () => ok(), eliminarDefinitivamente: () => ok(0), vaciarPapelera: () => ok(0), mover: () => ok(), duplicar: () => ok(elementos[0]), favorito: () => ok(lista[0]), historial: () => ok([]), restaurarVersion: () => ok(elementos[0]) },
  ...
  acceso: { buscar: () => ok(lista.filter((e) => e.favorito || e.usos > 5).concat(lista.slice(2, 6))), ocultar: () => ok(), abrirEnApp: () => ok(), abrirPrincipal: () => ok() },
  watchtower: { informe: () => ok(informe), filtradas: () => ok(informe) },
  ssh: { generar: () => ok({}) },
  web: { abrir: () => ok() },
  app: { version: () => ok('1.0.0') },
  ...
  en: { sesion: nada, datos: nada, ajustes: nada, menu: nada, abrirElemento: nada, accesoMostrado: nada, progresoFiltradas: nada }
})
